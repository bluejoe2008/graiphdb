package cn.graiph.storage.util
...
import org.neo4j.blob.{BlobId,Blob,BlobFactory,InputStreamSource,MimeTypeFactory}
...
class HBaseUtils(columnCount:Int=100 ) extends Logging {
  ...
  val colSize:Int = colCount/10  // size of 10;
  ...
  def cellDataToBlob(cellData:Array[Byte]):Blob={
    ...
    }, len, Some(MimeTypeFactory.fromCode(mimeTypeCode)))
    blob
  }
