...
class GraphDbBlobTest extends FunSuite with BeforeAndAfter with TestBase {
  ...
  test("test blob R/W using API") {
    ...
    assert(new File(s"$testBaseDir/testdata/test.png").length() == blob.length);

    assert(new File(s"$testBaseDir/testdata/test.png").length() == blob.offerStream {
    ...
    assert((0 to 5).toArray.map { x =>
      IOUtils.toByteArray(new FileInputStream(new File(s"$testBaseDir/testdata/test.png")))
    }
      ===
      blob2.map {
        _.offerStream {
  ...
  test("test blob using Cypher create") {
    ...
    db2.execute("CREATE (n {name:{NAME}, photo:{BLOB_OBJECT}})",
      JavaConversions.mapAsJavaMap(Map("NAME" -> "张三", "BLOB_OBJECT" -> BlobFactory.fromFile(new File(s"$testBaseDir/testdata/test1.png")))));
