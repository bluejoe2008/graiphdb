...
class BoltCypherTest extends FunSuite with BeforeAndAfter with TestBase {
  ...
  before {
    setupNewDatabase(new File(testBaseDir+"/testdata/testdb/data/databases/graph.db"));
  ...
  test("test blob R/W via cypher") {
    ...
    conn.querySingleObject(s"return Blob.fromFile('$testBaseDir/testdata/test.png')", (result: Record) => {
      val blob1 = result.get(0).asBlob
      assert(new File(s"$testBaseDir/testdata/test.png").length() == blob1.toBytes().length);
    ...
    conn.querySingleObject("match (n) where n.name='bob' return n.photo,n.album,Blob.len(n.photo) as len", (result: Record) => {
      ...
      assert(len == new File(s"$testBaseDir/testdata/test.png").length());
    ...
    conn.querySingleObject("match (n) where n.name='alex' return n.photo", (result: Record) => {
      ...
      assert(IOUtils.toByteArray(new FileInputStream(new File(s"$testBaseDir/testdata/test1.png"))) ===
        blob3.offerStream {
          IOUtils.toByteArray(_)
        });
    });
  ...
  test("test blob R/W via blob literal") {
    ...
    assert(conn.querySingleObject(s"return Blob.len(<file://$testBaseDir/testdata/test.png>)", (result: Record) => {
      result.get(0).asInt
    }) == new File(s"$testBaseDir/testdata/test.png").length());
  }
}
